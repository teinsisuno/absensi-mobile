...
- **Face recognition (2026-08)**: embedding dihitung di device dengan TensorFlow.js (`rn-webgl` via `expo-gl`) + model `faceres` 128-d dari vladmandic/human (di-bundle di `assets/models/face-embed/`, single-shard ~7MB). `SetupFaceScreen` & `FaceEnrollScreen` ambil foto selfie → crop persegi tengah (guide oval) → resize 224x224 → embedding 128-d → `POST /face/enroll` `{ template: JSON, mode: 'client' }`. Matching (Euclidean, threshold 0.6) tetap di server via `FaceRecognitionService`. Model di-load sekali (singleton) & di-dispose saat clear cache.
